...
// Watch for changes in selectedAspectIds
watch(
    selectedAspectIds,
    (newIds: number[]) => {
        // Update form.selected_aspects based on current selections
        const updatedAspects: { id: number; weight: number }[] = [];
        
        newIds.forEach((aspectId: number) => {
            const existingAspect = form.selected_aspects.find((a) => a.id === aspectId);
            const templateAspect = props.template.latest_version?.aspect_versions?.find(av => av.aspect_id === aspectId);
            
            updatedAspects.push({
                id: aspectId,
                weight: existingAspect ? existingAspect.weight : (templateAspect?.pivot?.weight || 0),
            });
        });
        
        form.selected_aspects = updatedAspects;
    },
    { deep: true }
);
...
const updateAspectWeight = (aspectId: number, weight: number) => {
    ...
    if (aspectIndex > -1) {
        ...
        // If aspect not found, add it
        form.selected_aspects.push({ id: aspectId, weight });
    }
};
        ...
        <div class="py-6 lg:py-8">
            <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
                ...
                <form @submit.prevent="submit" class="space-y-8">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>Pemilihan Aspek & Pembobotan</CardTitle>
                        </CardHeader>
                        <CardContent class="space-y-6">
                                    ...
                                    <div v-for="aspect in props.aspects" :key="aspect.id" class="flex items-center justify-between rounded-lg border p-4">
                                        <div class="flex items-center space-x-3">
                                            <Checkbox
                                                ...
                                                @update:checked="(checked: any) => {
                                                    if (checked) {
                                                        selectedAspectIds.push(aspect.id);
                                                    } else {
                                                        const index = selectedAspectIds.indexOf(aspect.id);
                                                        if (index > -1) selectedAspectIds.splice(index, 1);
                                                    }
                                                }"
                                            />
                                        ...
                                        <div v-if="selectedAspectIds.includes(aspect.id)" class="flex items-center space-x-2">
                                            ...
                                            <Input
                                                ...
                                                @input="(e: any) => {
                                                    const aspectIndex = form.selected_aspects.findIndex(a => a.id === aspect.id);
                                                    if (aspectIndex > -1) {
                                                        form.selected_aspects[aspectIndex].weight = parseFloat(e.target.value) || 0;
                                                    }
                                                }"
                                                class="w-20"
                                            />
